package com.squad_stack.repository.impl;

import com.squad_stack.exception.SlotNotAvailableException;
import com.squad_stack.exception.VehicleNotFoundException;
...
public class ParkingSlotRepositoryImpl implements InMemoryParkingSlotRepository {
    ...
    public int getSlotOfVehicleNumber(String vehicleNumber) {
        for(int i =0; i<numberOfParking.size(); i++){
            if(numberOfParking.get(i) != null) {
                if (Objects.equals(numberOfParking.get(i).getVehicle().getVehicleNumber(), vehicleNumber)) {
                    return i;
                }
            }
        }
